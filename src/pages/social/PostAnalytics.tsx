            ...
            <div key={row.p} className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0 flex flex-col">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs text-slate-500">{row.followers.toLocaleString()}</span>
                </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div className="max-h-[340px] overflow-y-auto">
          {visiblePosts.map((post) => (
            <div key={post.id} className="p-4 border-b border-slate-50 last:border-0">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-slate-400 flex items-center gap-1"><Clock className="w-3 h-3" />{post.ago}</span>
                <span className={`ml-auto text-[10px] font-bold px-1.5 py-0.5 rounded-full ${post.aiScore >= 90 ? 'bg-green-100 text-green-700' : post.aiScore >= 75 ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600'}`}>
                  {post.aiScore}
                </span>
              </div>
              <p className="text-xs text-slate-700 line-clamp-2 leading-relaxed mb-2">{post.caption}</p>
          ...
          {visiblePosts.length === 0 && (
            <p className="text-xs text-slate-400 text-center py-8">No posts for this platform</p>
          )}
        </div>
      </div>
    </div>
