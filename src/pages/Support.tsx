  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Support Command Desk</h1>
          <p className="text-sm text-gray-500">ZeroBT Grievance Redressal, Frustration Radar & Multi-Tier SLA Escalation.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
          <p className="text-xs text-gray-500 font-semibold">Active Incidents</p>
          <p className="text-2xl font-bold text-gray-900 mt-1">{openCount}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
          <p className="text-xs text-gray-500 font-semibold">Avg Frustration Index</p>
          <p className="text-2xl font-bold text-orange-600 mt-1">54/100</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
          <p className="text-xs text-gray-500 font-semibold">Avg Response SLA</p>
          <p className="text-2xl font-bold text-emerald-600 mt-1">12 min</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
          <p className="text-xs text-gray-500 font-semibold">ZeroBT Auto-Triage</p>
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100 text-[11px] uppercase tracking-wider text-gray-500 text-left">
              ...
              <tr key={ticket.id} className="hover:bg-gray-50/80 transition-colors">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-gray-900 text-xs">{ticket.id}</span>
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-blue-50 text-blue-700 font-semibold">{ticket.client}</span>
                  </div>
                  <p className="text-xs text-gray-600 mt-0.5">{ticket.title}</p>
                </td>
                <td className="px-4 py-3 text-xs text-gray-600">{ticket.area}</td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] font-mono font-bold text-gray-700">{ticket.frustrationScore}/100</span>
                  </div>
                </td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3">
                  <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full ${ticket.status === 'Resolved' ? 'bg-green-100 text-green-700' : ticket.status === 'Investigating' ? 'bg-violet-100 text-violet-700' : 'bg-gray-100 text-gray-700'}`}>
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 max-w-xl w-full p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-gray-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-gray-900 text-sm">Escalation Dossier — {activeTicket.id}</h3>
              </div>
              <button onClick={() => setShowDossierModal(false)} className="text-gray-400 hover:text-gray-600">
                <X size={18} />
              </button>
            </div>
            ...
            {isDossierLoading ? (
              <div className="py-10 flex flex-col items-center justify-center space-y-2">
                ...
                <p className="text-xs text-gray-500">Generating ZeroBT Resolution Protocol...</p>
              </div>
            ) : dossier ? (
              <div className="space-y-3 text-xs">
                <div className="p-3 bg-orange-50 rounded-lg border border-orange-200">
                  <span className="font-bold text-orange-800 block mb-0.5">Executive Brief</span>
                  <p className="text-gray-800">{dossier.executiveBrief}</p>
                </div>
                <div className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                  <span className="font-bold text-gray-700 block mb-0.5">Recommended Tactical Action</span>
                  <p className="text-gray-600">{dossier.recommendedAction}</p>
                </div>
      ...
      {showCreate ? (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="w-full max-w-md rounded-xl bg-white border border-gray-200 shadow-xl p-5 space-y-4">
            <h3 className="text-lg font-semibold text-gray-900">Create Support Incident Ticket</h3>
            <div>
              <label className="text-xs font-semibold text-gray-600">Client / Account Name</label>
              <input
                value={ticketClient}
