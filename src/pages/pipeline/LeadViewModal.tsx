...
export default function LeadViewModal({ viewLead, setViewId, openEdit }: LeadViewModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4 overlay-enter">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden modal-enter border border-slate-200">
        {/* Modal Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-3">
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-bold text-slate-900">{viewLead.name}</h2>
                {viewLead.isHot && (
              ...
              <p className="text-xs text-slate-500 flex items-center gap-2 mt-0.5">
                {viewLead.company && <span className="font-semibold text-slate-700">{viewLead.company}</span>}
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setViewId(null)} className="p-1 rounded-lg hover:bg-slate-200 text-slate-400 hover:text-slate-700">
              <X size={18} />
            </button>
          </div>
        </div>
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4 text-xs">
                <div>
                  ...
                  <p className="font-semibold text-slate-800 font-mono mt-0.5">{viewLead.phone}</p>
                </div>
                <div>
                  <p className="text-slate-400 text-[11px]">Email Address</p>
                  <p className="font-semibold text-slate-800 mt-0.5">{viewLead.email}</p>
                </div>
                <div>
                  <p className="text-slate-400 text-[11px]">Company</p>
                  <p className="font-semibold text-slate-800 mt-0.5">{viewLead.company ?? '—'}</p>
                </div>
                <div>
                  <p className="text-slate-400 text-[11px]">Designation</p>
                  <p className="font-semibold text-slate-800 mt-0.5">{viewLead.designation ?? '—'}</p>
                </div>
                <div>
                  <p className="text-slate-400 text-[11px]">City / Location</p>
                  <p className="font-semibold text-slate-800 mt-0.5">{viewLead.city ?? '—'}</p>
                </div>
                <div>
                  <p className="text-slate-400 text-[11px]">Budget Size</p>
                  <p className="font-bold text-slate-900 font-mono mt-0.5">{viewLead.budget ?? '—'}</p>
                </div>
                <div>
                  <p className="text-slate-400 text-[11px] mb-1">Lead Source</p>
                  <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${sourceBadge[viewLead.source]}`}>
                    {viewLead.source}
                  </span>
                </div>
                <div>
                  <p className="text-slate-400 text-[11px] mb-1">Deal Stage</p>
                  <span
                ...
                <div>
                  <p className="text-slate-400 text-[11px]">Assigned Sales Rep</p>
                  <p className="font-semibold text-slate-800 mt-0.5">{viewLead.assignedTo || 'Unassigned'}</p>
                </div>
                <div>
                  <p className="text-slate-400 text-[11px]">Created At</p>
                  <p className="font-semibold text-slate-800 mt-0.5">{viewLead.createdAt}</p>
                </div>
              </div>
                ...
                {(['sourceQuality', 'recency', 'profileCompleteness'] as const).map((k) => (
                  <div key={k} className="mb-2 last:mb-0">
                    <div className="flex items-center justify-between text-[11px] mb-0.5 font-medium">
                      ...
                      <span className="font-bold text-slate-700">{viewLead.aiScoreBreakdown[k]} pts</span>
                    </div>
              ...
              {viewLead.notes && (
                <div>
                  <p className="text-xs font-bold text-slate-700 mb-1">Lead Notes</p>
        ...
        <div className="flex justify-between items-center px-6 py-3.5 border-t border-slate-100 bg-slate-50/50">
          <span className="text-[11px] text-slate-400">ID: {viewLead.id}</span>
          <div className="flex gap-2">
            <button
              onClick={() => {
                const l = viewLead;
