    ...
    <>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            ...
            <input value={crm.companyName} onChange={(e) => setCrm({ ...crm, companyName: e.target.value })} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-600 block mb-1">Business Email</label>
            <input type="email" value={crm.companyEmail} onChange={(e) => setCrm({ ...crm, companyEmail: e.target.value })} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-600 block mb-1">Phone Number</label>
            <input value={crm.companyPhone} onChange={(e) => setCrm({ ...crm, companyPhone: e.target.value })} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-600 block mb-1">Industry</label>
            <select value={crm.industry} onChange={(e) => setCrm({ ...crm, industry: e.target.value })} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
          ...
          <div>
            ...
            <input value={crm.city} onChange={(e) => setCrm({ ...crm, city: e.target.value })} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div className="col-span-2">
            <label className="text-xs font-semibold text-slate-600 block mb-1">Website URL</label>
            <input value={crm.website} onChange={(e) => setCrm({ ...crm, website: e.target.value })} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-base font-semibold text-slate-900">Lead Management Settings</h2>
        </div>
        <div className="flex flex-col gap-5">
          {/* HOT Threshold */}
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-slate-800">HOT Lead Score Threshold</p>
              <p className="text-xs text-slate-500 mt-0.5">Leads scoring above this are marked HOT 🔥</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-slate-800">Auto Follow-up Timing</p>
              <p className="text-xs text-slate-500 mt-0.5">Default reminder timing for follow-ups</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">Automatically assign leads to available reps</p>
            </div>
            <Toggle on={crm.autoAssign} onToggle={() => setCrm({ ...crm, autoAssign: !crm.autoAssign })} />
          </div>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-slate-800">Round-robin assignment</p>
              <p className="text-xs text-slate-500 mt-0.5">Distribute leads equally across all reps</p>
            </div>
          ...
          <div>
            <p className="text-sm font-medium text-slate-800 mb-3">Lead Scoring Weights by Source</p>
            {([
              ...
              <div key={s.key} className="flex items-center gap-3 mb-3 last:mb-0">
                ...
                <span className="text-sm font-medium text-slate-700 w-12 text-right">{crm[s.key]}/100</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-base font-semibold text-slate-900">Velara AI Settings</h2>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div>
            <p className="text-xs font-medium text-slate-600 mb-2">AI Response Language</p>
            <div className="flex gap-2">
              {['English','Hindi','Hinglish'].map((v) => (
                <button key={v} onClick={() => saveCrm({ ...crm, aiLang: v })}
                  className={`px-4 py-2 rounded-lg border text-sm font-medium transition-colors ${crm.aiLang === v ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-600 border-slate-200 hover:border-slate-300'}`}>
                  {v}
                </button>
              ))}
            </div>
