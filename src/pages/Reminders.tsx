...
export default function Reminders() {
  ...
  const toggleReminderCompleted = useCrmStore((s) => s.toggleReminderCompleted);
  const deleteReminder = useCrmStore((s) => s.deleteReminder);
  const updateReminder = useCrmStore((s) => s.updateReminder);
  ...
  function handleSave() {
    ...
    if (editId) {
      updateReminder(editId, {
        leadName: form.leadName, task: form.task, dueDate: form.dueDate, dueTime: form.dueTime, priority: form.priority, type: form.type, isToday: isToday(form.dueDate), isTomorrow: isTomorrow(form.dueDate)
      });
    } else {
      const newR: Reminder = {
        id: 'rem_' + Date.now(),
    ...
    <div className="page-stack">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2 space-y-6 min-w-0">
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-lg font-bold text-slate-900">Active Reminders</h2>
            </div>

            {/* Filter tabs */}
            <div className="flex gap-1 mb-4 flex-wrap">
              {TABS.map((t) => (
                <button key={t} onClick={() => { setTab(t); setCalDate(null); }} className={`text-xs font-medium px-3 py-1.5 rounded-full transition-colors ${tab === t ? 'bg-[#2563EB] text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                  {t}
                </button>
              ))}
            </div>
            ...
            {filtered.length === 0 ? (
              ...
              <div className="space-y-3">
                {filtered.map((r) => {
                  ...
                  const dueLabelColor = isToday(r.dueDate) ? 'bg-red-100 text-red-700' : isTomorrow(r.dueDate) ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600';
                  return (
                    <div key={r.id} className={`flex items-start gap-3 bg-white rounded-xl shadow-sm border border-slate-100 p-4 border-l-4 ${completed ? 'border-l-green-400 opacity-60' : priorityBorder[r.priority]}`}>
                      {/* checkbox */}
                      <div className="pt-0.5 shrink-0">
                        {completed ? (
                          <CheckCircle size={20} className="text-green-500" />
                        ) : (
                          <button onClick={() => handleComplete(r.id)} className="w-5 h-5 rounded border-2 border-slate-300 hover:border-blue-500 transition-colors" />
                        )}
                      </div>
                      {/* info */}
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm font-medium ${completed ? 'line-through text-slate-400' : 'text-slate-800'}`}>{r.task}</p>
                        <p className="text-xs text-blue-600 mt-0.5 cursor-pointer hover:underline">{r.leadName}</p>
                        <span className={`inline-block mt-1 text-[10px] font-semibold px-2 py-0.5 rounded-full ${r.type === 'AI-Generated' ? 'bg-purple-100 text-purple-700' : 'bg-slate-100 text-slate-500'}`}>{r.type}</span>
                      </div>
                      {/* right meta */}
                      <div className="flex flex-col items-end gap-1.5 shrink-0 text-right">
                        ...
                        {!completed && (
                          <div className="flex items-center gap-1 mt-0.5">
                            <button onClick={() => openEdit(r)} className="p-1 rounded hover:bg-slate-100 text-slate-400 transition-colors"><Pencil size={13} /></button>
                            <button onClick={() => handleDelete(r.id)} className="p-1 rounded hover:bg-red-50 text-slate-400 hover:text-red-500 transition-colors"><Trash2 size={13} /></button>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h2 className="text-lg font-bold text-slate-900">AI Follow-up Rules</h2>
            </div>
            <p className="text-xs text-slate-400 mb-3">Velara AI automatically creates reminders based on these rules</p>
        ...
        <div className="space-y-6 min-w-0">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4">
            <div className="flex items-center justify-between gap-2 flex-wrap mb-3">
              ...
              <div className="flex items-center gap-1 shrink-0 ml-auto">
                <button onClick={() => { if (calMonth === 0) { setCalMonth(11); setCalYear(calYear - 1); } else setCalMonth(calMonth - 1); }} className="p-1 rounded hover:bg-slate-100 text-slate-500"><ChevronLeft size={16} /></button>
                <span className="text-xs font-semibold text-slate-700 w-24 text-center">{new Date(calYear, calMonth).toLocaleDateString('en-IN', { month: 'long', year: 'numeric' })}</span>
                <button onClick={() => { if (calMonth === 11) { setCalMonth(0); setCalYear(calYear + 1); } else setCalMonth(calMonth + 1); }} className="p-1 rounded hover:bg-slate-100 text-slate-500"><ChevronRight size={16} /></button>
              </div>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`relative flex flex-col items-center py-1 rounded-lg text-xs transition-colors ${
                      isTod ? 'bg-[#2563EB] text-white font-bold' : isSelected ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-slate-700 hover:bg-slate-100'
                    }`}
                  >
                    {d}
                    {dotColor && (
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-900 mb-2">This Week's Overview</h3>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-bold text-slate-900">Recently Completed</h3>
            </div>
            {recentCompleted.length === 0 ? (
              <p className="text-xs text-slate-400 text-center py-4">No completed reminders yet.</p>
            ) : (
              <div className="space-y-2">
                {recentCompleted.map((r) => (
                  <div key={r.id} className="flex items-center gap-2 text-xs">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-slate-700 truncate">{r.leadName} — {r.task}</p>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 overlay-enter">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 overflow-hidden modal-enter">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              ...
              <button onClick={() => { setShowModal(false); setEditId(null); }} className="p-1 rounded-lg hover:bg-slate-100 text-slate-500"><X size={18} /></button>
            </div>
            ...
            <div className="px-5 py-4 space-y-3 max-h-[60vh] overflow-y-auto">
              <div>
                ...
                <input type="text" value={form.leadName} onChange={(e) => setForm({ ...form, leadName: e.target.value })} placeholder="Search lead…" className="w-full mt-1 px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
                <label className="text-xs font-semibold text-slate-700">Task Description *</label>
                <textarea rows={2} value={form.task} onChange={(e) => setForm({ ...form, task: e.target.value })} placeholder="Follow up on…" className="w-full mt-1 px-3 py-2 text-sm border border-slate-200 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-slate-700">Due Date *</label>
                  <input type="date" value={form.dueDate} onChange={(e) => setForm({ ...form, dueDate: e.target.value })} className="w-full mt-1 px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-slate-700">Due Time *</label>
                  <input type="time" value={form.dueTime} onChange={(e) => setForm({ ...form, dueTime: e.target.value })} className="w-full mt-1 px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
              </div>
              <div>
                ...
                <div className="flex gap-3 mt-1">
                  {(['High', 'Medium', 'Low'] as const).map((p) => (
                    <label key={p} className={`flex items-center gap-1.5 text-xs font-medium cursor-pointer px-3 py-1.5 rounded-lg border transition-colors ${form.priority === p ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
              ...
              <div>
                ...
                <div className="flex gap-3 mt-1">
                  {(['Manual', 'AI-Generated'] as const).map((t) => (
                    <label key={t} className={`flex items-center gap-1.5 text-xs font-medium cursor-pointer px-3 py-1.5 rounded-lg border transition-colors ${form.type === t ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
              ...
              <div>
                ...
                <textarea rows={2} value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Additional notes…" className="w-full mt-1 px-3 py-2 text-sm border border-slate-200 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
            </div>

            {/* footer */}
            <div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-slate-100">
              <button onClick={() => { setShowModal(false); setEditId(null); }} className="px-4 py-2 text-sm font-medium text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">Cancel</button>
